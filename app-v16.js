...
function toggleProfileSection(id){const el=document.getElementById(id);if(el)el.classList.toggle("hidden");}
function msftHistoryChart(){
 const rows=[
  ["Revenue",[211.9,245.1,281.7,331.8],"B"],
  ["Operating income",[88.5,109.4,128.5,155.2],"B"],
  ["Net income",[72.4,88.1,101.8,133.7],"B"],
  ["Diluted EPS",[9.68,11.80,13.64,17.95],"$"]
 ];
 return rows.map((r,i)=>{
   const max=Math.max(...r[1]);
   return `<div class="metric-chart"><div class="chart-label"><b>${r[0]}</b><span>FY23 → FY26</span></div><div class="chart-bars">${r[1].map((v,j)=>`<div class="chart-col"><div class="chart-value">${r[2]==="$"?"$":""}${v.toFixed(r[2]==="$"?2:1)}${r[2]==="B"?"B":""}</div><div class="chart-bar"><i style="height:${Math.max(8,v/max*100)}%"></i></div><div class="chart-year">FY${23+j}</div></div>`).join("")}</div></div>`;
 }).join("");
}
function stock(t){
 ...
 <div class="profile-section"><button class="profile-toggle" onclick="toggleProfileSection('${finId}')"><b>Financials</b><span>＋</span></button><div id="${finId}" class="profile-content hidden">
   ...
   <div class="card" style="margin-top:10px"><div class="label">Growth visualization</div>${msftHistoryChart()}</div>
 ...
 <div class="profile-section"><button class="profile-toggle" onclick="toggleProfileSection('${segId}')"><b>Business segments</b><span>＋</span></button><div id="${segId}" class="profile-content hidden">${r.segments.map(a=>`<div class="list-item"><b>${a[0]}</b><span class="muted">${a[1]}</span></div>`).join("")}</div></div>
 <div class="profile-section"><button class="profile-toggle" onclick="toggleProfileSection('${thesisId}')"><b>Investment thesis</b><span>＋</span></button><div id="${thesisId}" class="profile-content hidden">${r.thesis.map(a=>`<div class="list-item"><span>${a}</span></div>`).join("")}</div></div>
 <div class="profile-section"><button class="profile-toggle" onclick="toggleProfileSection('${riskId}')"><b>Key risks</b><span>＋</span></button><div id="${riskId}" class="profile-content hidden">${r.risks.map(a=>`<div class="list-item"><span>${a}</span></div>`).join("")}</div></div>
 </div>`:""}
