...
function msftHistoryChart(){
 const metrics=[
  {name:"Revenue",unit:"$B",v:[211.9,245.1,281.7,331.8]},
  {name:"Operating income",unit:"$B",v:[88.5,109.4,128.5,155.2]},
  {name:"Net income",unit:"$B",v:[72.4,88.1,101.8,133.7]},
  {name:"Diluted EPS",unit:"$",v:[9.68,11.80,13.64,17.95]}
 ];
 return metrics.map((m,i)=>{
   const min=Math.min(...m.v), max=Math.max(...m.v), range=max-min||1;
   const pts=m.v.map((v,j)=>`${j*33.33},${96-((v-min)/range)*76}`).join(" ");
   const dots=m.v.map((v,j)=>`<circle cx="${j*33.33}" cy="${96-((v-min)/range)*76}" r="2.8"></circle>`).join("");
   const last=m.v[3], growth=((last/m.v[0])-1)*100;
   return `<div class="metric-visual"><div class="metric-top"><b>${m.name}</b><span>${m.unit===" $"?"$":""}${m.unit==="$"?last.toFixed(2):last.toFixed(1)}${m.unit==="B"?"B":""} <em>${growth>=0?"+":""}${growth.toFixed(0)}% since FY23</em></span></div><svg viewBox="0 0 100 110" preserveAspectRatio="none" class="sparkline"><polyline points="${pts}" fill="none" stroke="currentColor" stroke-width="2.5" vector-effect="non-scaling-stroke"></polyline>${dots}</svg><div class="years"><span>FY23</span><span>FY24</span><span>FY25</span><span>FY26</span></div></div>`;
 }).join("");
}
function stock(t){
