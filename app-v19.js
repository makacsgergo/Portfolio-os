...
function msftHistoryChart(){
 ...
 return metrics.map(m=>{
   const max=Math.max(...m.v);
   const growth=((m.v[3]/m.v[0])-1)*100;
   return `<div class="metric-visual"><div class="metric-top"><b>${m.name}</b><span>${m.unit==="$"?"$":""}${m.v[3].toFixed(m.unit==="$"?2:1)}${m.unit==="B"?"B":""} <em>+${growth.toFixed(0)}% since FY23</em></span></div><div class="metric-bars">${m.v.map((v,j)=>`<div class="metric-bar-col"><div class="metric-bar-value">${m.unit==="$"?"$":""}${v.toFixed(m.unit==="$"?2:1)}${m.unit==="B"?"B":""}</div><div class="metric-bar-track"><div class="metric-bar-fill" style="height:${Math.max(8,(v/max)*100)}%"></div></div><div class="metric-bar-year">FY${23+j}</div></div>`).join("")}</div></div>`;
 }).join("");
}
function stock(t){
