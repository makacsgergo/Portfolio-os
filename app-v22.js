...
function msftHistoryChart(){
 const metrics=[
  {name:"Revenue",v:[211.9,245.1,281.7,331.8],unit:"B"},
  {name:"Operating income",v:[88.5,109.4,128.5,155.2],unit:"B"},
  {name:"Net income",v:[72.4,88.1,101.8,133.7],unit:"B"},
  {name:"Diluted EPS",v:[9.68,11.80,13.64,17.95],unit:"$"},
  {name:"Free cash flow",v:[59.5,74.1,71.6,67.0],unit:"B"}
 ];
 return metrics.map(m=>{
  const max=Math.max(...m.v), growth=((m.v[3]/m.v[0])-1)*100;
  return '<div style="margin:16px 0;padding:12px;border:1px solid #303b4d;border-radius:10px;background:#0f1722">'+
   '<div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:10px"><b>'+m.name+'</b><span>'+ (m.unit==="$"?"$":"")+m.v[3].toFixed(m.unit==="$"?2:1)+(m.unit==="B"?"B":"")+' · +'+growth.toFixed(0)+'% since FY23</span></div>'+
   '<div style="display:flex;align-items:flex-end;gap:8px;height:130px">'+
   m.v.map((v,j)=>'<div style="flex:1;height:100%;display:flex;flex-direction:column;justify-content:flex-end;align-items:center"><div style="font-size:10px;margin-bottom:4px">'+(m.unit==="$"?"$":"")+v.toFixed(m.unit==="$"?2:1)+(m.unit==="B"?"B":"")+'</div><div style="width:70%;height:'+Math.max(8,(v/max)*100)+'px;background:#5b9cff;border-radius:5px 5px 0 0"></div><div style="font-size:10px;opacity:.6;margin-top:5px">FY'+(23+j)+'</div></div>').join("")+
   '</div></div>';
 }).join("");
}
function stock(t){
